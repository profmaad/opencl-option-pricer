...
float arithmetic_basket_expected_underlying_price_at_maturity(float start_price, float risk_free_rate, float maturity)
{
	return exp(risk_free_rate * maturity) * start_price;
}

/* float arithmetic_basket_geometric_cv_adjusted_strike(float start_price, float strike_price, float maturity, float volatility, float risk_free_rate, unsigned int averaging_steps) */
/* { */
/* 	float geometric_expectation = geometric_basket_expected_underlying_price_at_maturity(start_price, maturity, volatility, risk_free_rate, averaging_steps); */
/* 	float arithmetic_expectation = arithmetic_basket_expected_underlying_price_at_maturity(start_price, risk_free_rate, maturity); */
...
/* __kernel void arithmetic_basket_geometric_cv(unsigned int direction, float start_price, float strike_price, float maturity, float volatility, float risk_free_rate, unsigned int averaging_steps, unsigned int total_number_of_paths, unsigned int adjust_strike, __global uint2 *seeds, __global float2 *arithmetic_results, __global float2 *geometric_results, __global float *arithmetic_geometric_means) */
/* { */
/* 	// get details on worker setup */
/* 	unsigned int tid = get_global_id(0); */
/* 	unsigned int worker_count = get_global_size(0); */
...
/* 	float adjusted_strike_price = (adjust_strike == 0 ? strike_price : arithmetic_basket_geometric_cv_adjusted_strike(start_price, strike_price, maturity, volatility, risk_free_rate, averaging_steps)); */
